  ...
  async.mapLimit(
    ...
    function (fund, callback) {
      ...
      FundDB.queryFund(fund.code, function (resp) {
        Log.info(
          'Finish queryFund task ' + taskTimes + 'data:' + resp.fundByCode
        )
        ...
        if (resp.fundByCode == null) {
          ...
          FundIncreaseDB.updateFundIncrease(fund, function (resp) {
            Log.success(
              'Finish updateFundIncrease task: ' + resp.updateFundIncrease
            )
            // console.log(resp.fundByCode)
          })
          callback(null, null)
    ...
    function (err, results) {
      ...
      async.mapLimit(createFundList, 1, async function (fund) {
        if (fund != null) {
          Log.info(
            'Start waitList task:' + waitListTimes + '/' + waitListLength
          )
          // 创建新基金
